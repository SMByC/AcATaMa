...
from AcATaMa.utils.others_utils import get_epsilon, get_nodata_format
from AcATaMa.utils.qgis_utils import get_source_from, load_and_select_layer_in, valid_file_selected_in
from AcATaMa.utils.system_utils import error_handler, get_save_file_name, output_file_is_OK
...
def do_simple_random_sampling():
    ...
    if sampling_design.QGBox_SimpRSwithPS.isChecked():
        if not valid_file_selected_in(sampling_design.QCBox_PostStratMap_SimpRS, "post-stratification map"):
            sampling_design.MsgBar.pushMessage(
                "Error, post-stratification option is enabled but not configured",
    ...
    if sampling_design.QGBox_SimpRSwithPS.isChecked():
        ...
        try:
            ...
            sampling_design.MsgBar.pushMessage(
                "Error, post-stratification option is enabled but none of the classes were selected",
                level=Qgis.MessageLevel.Warning,
                duration=10,
            )
            return
    ...
    suggested_filename = get_source_from(AcATaMa.dockwidget.QCBox_SamplingFile) or os.path.join(
        os.path.dirname(thematic_map.file_path),
    ...
    if (
        sampling_design.QGBox_random_sampling_options_SimpRS.isChecked()
        and sampling_design.with_random_seed_by_user_SimpRS.isChecked()
    ):
    ...
    sampling = Sampling(
        "simple", thematic_map, post_stratification_map=post_stratification_map, output_file=output_file
    )
    sampling_conf = {
    ...
    globals()["sampling_task"] = QgsTask.fromFunction(
        ...
        on_finished=simple_random_sampling_finished,
...
def simple_random_sampling_finished(exception, result=None):
    ...
    if exception is not None or result is None:
        raise Exception(f"Error in sampling process: {exception}")
    ...
    if not sampling_layer_generated:
        sampling_design.MsgBar.pushMessage(
            f"Failed to load the generated sampling file: {sampling.output_file}",
    ...
    if sampling.samples_generated == sampling_conf["total_of_samples"]:
        sampling_report.MsgBar.pushMessage(
            f"Simple random sampling successful: {sampling.samples_generated} samples generated",
    ...
    if sampling_conf["total_of_samples"] > sampling.samples_generated > 0:
        sampling_report.MsgBar.pushMessage(
            "Simple random sampling successful: {} samples generated from {}".format(
    ...
    if sampling_conf["min_distance"] > 0:
        if sampling.thematic_map.qgs_layer.crs().mapUnits() == QgsUnitTypes.DistanceUnit.DistanceUnknownUnit:
            sampling_report.MsgBar.pushMessage(
                ...
                f" as the base unit to calculate the minimum distances",
                level=Qgis.MessageLevel.Warning,
                duration=20,
            )
...
def do_stratified_random_sampling():
    ...
    if total_of_samples == 0:
        sampling_design.MsgBar.pushMessage(
            "Error, no number of samples configured!", level=Qgis.MessageLevel.Warning, duration=10
        )
        return
    ...
    # first select the target file for save the sampling file
    suggested_filename = get_source_from(AcATaMa.dockwidget.QCBox_SamplingFile) or os.path.join(
        os.path.dirname(thematic_map.file_path), "stratified sampling.gpkg"
    )
    ...
    # define the random seed
    if (
        sampling_design.QGBox_random_sampling_options_StraRS.isChecked()
        and sampling_design.with_random_seed_by_user_StraRS.isChecked()
    ):
    ...
    sampling_conf = {
        ...
        "total_of_samples": total_of_samples_by_stratum,
        "min_distance": min_distance,
    ...
    globals()["sampling_task"] = QgsTask.fromFunction(
        ...
        sampling.generate_sampling_points,
        on_finished=stratified_random_sampling_finished,
        sampling_conf=sampling_conf,
    )
...
def stratified_random_sampling_finished(exception, result=None):
    ...
    if sampling.samples_generated == sum(sampling_conf["total_of_samples"]):
        sampling_report.MsgBar.pushMessage(
            f"Stratified random sampling successful: {sampling.samples_generated} samples generated",
            level=Qgis.MessageLevel.Success,
            duration=20,
        )
    # success but not completed
    if sampling.samples_generated < sum(sampling_conf["total_of_samples"]) and sampling.samples_generated > 0:
        sampling_report.MsgBar.pushMessage(
            "Stratified random sampling successful: {} samples generated from {}".format(
...
def do_systematic_sampling():
    ...
    if sampling_design.QGBox_SystSwithPS.isChecked():
        if not valid_file_selected_in(sampling_design.QCBox_PostStratMap_SystS, "post-stratification map"):
            sampling_design.MsgBar.pushMessage(
                "Error, post-stratification option enabled but not configured",
                level=Qgis.MessageLevel.Warning,
                duration=10,
            )
            return
    ...
    # define the random seed
    if (
        sampling_design.QGBox_random_sampling_options_SystS.isChecked()
        and sampling_design.with_random_seed_by_user_SystS.isChecked()
    ):
    ...
    sampling = Sampling(
        "systematic",
        thematic_map,
        post_stratification_map=post_stratification_map,
        sampling_method="grid with random offset",
        output_file=output_file,
    )
    sampling_conf = {
    ...
    globals()["sampling_task"] = QgsTask.fromFunction(
        "Systematic sampling",
        systematic_sampling_function,
        on_finished=systematic_sampling_finished,
        sampling_conf=sampling_conf,
    )
...
def systematic_sampling_finished(exception, result=None):
    ...
    if sampling.samples_generated == sampling_conf["total_of_samples"]:
        sampling_report.MsgBar.pushMessage(
            f"Systematic random sampling successful: {sampling.samples_generated} samples generated",
            level=Qgis.MessageLevel.Success,
            duration=20,
        )
    # success but not completed
    if sampling_conf["total_of_samples"] > sampling.samples_generated > 0:
        sampling_report.MsgBar.pushMessage(
            "Systematic random sampling successful: {} samples generated from {}".format(
                sampling.samples_generated, sampling_conf["total_of_samples"]
            ),
            level=Qgis.MessageLevel.Success,
            duration=20,
...
class Sampling:
    ...
    def generate_sampling_points(self, task, sampling_conf):
        ...
        file_format = (
            "GPKG"
            if self.output_file.endswith(".gpkg")
            else "ESRI Shapefile"
            if self.output_file.endswith(".shp")
            else None
        )
    ...
    def generate_systematic_sampling_points_by_pixels(self, task, sampling_conf):
        ...
        while not task.isCanceled() and y >= self.thematic_map.extent().yMinimum() - pixel_size_y * self.points_spacing:
            ...
            while (
                not task.isCanceled()
                and x <= self.thematic_map.extent().xMaximum() + pixel_size_x * self.points_spacing
            ):
                ### aligned sampling without offset
                if self.max_xy_offset == 0:
                ...
                ### with random offset
                # define the offset grid area by each aligning grid point
                x_offset_grid = np.arange(
                    x - pixel_size_x * self.max_xy_offset,
                    x + pixel_size_x * self.max_xy_offset - pixel_size_x / 2,
                    pixel_size_x,
                )
                y_offset_grid = np.arange(
                    y - pixel_size_y * self.max_xy_offset + pixel_size_y,
                    y + pixel_size_y * self.max_xy_offset + pixel_size_y / 2,
                    pixel_size_y,
                )
                ...
                for x_offset in x_offset_grid:
                    for y_offset in y_offset_grid:
                        ...
                        _x, _y = self.thematic_map.get_pixel_centroid(
                            x_offset + pixel_size_x / 2, y_offset - pixel_size_y / 2
                        )
    ...
    def check_sampling_point(self, sampling_point):
        ...
        if self.sampling_design_type == "stratified":
            if not sampling_point.in_max_samples_in_stratum(
                self.classes_for_sampling, self.total_of_samples, self.sampling_map, self.samples_in_strata
            ):
                return False

        return not (
            self.neighbor_aggregation
            and not sampling_point.check_neighbors_aggregation(self.thematic_map, *self.neighbor_aggregation)
        )
