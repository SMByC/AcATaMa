...
from qgis.PyQt.QtWidgets import QApplication, QFileDialog, QMessageBox, QPushButton
...
def error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            msg_bar = ResponseDesignWindow.inst.MsgBar if ResponseDesignWindow.is_opened else iface.messageBar()
            ...
            def details_message_box(error, more_details):
                ...
                msgBox.setInformativeText(
                    "If you consider this as an AcATaMa error, report it in "
...
def output_file_is_OK(output_file):
    ...
    if not os.path.exists(os.path.dirname(output_file)):
        QMessageBox.critical(
            None,
            "AcATaMa",
            f"Error: The output file path does not exist:\n\n {os.path.dirname(output_file)}",
    ...
    if not os.access(os.path.dirname(output_file), os.W_OK):
        QMessageBox.critical(
            None,
            "AcATaMa",
            f"Error: The output file path is not writable:\n\n {output_file}",
            QMessageBox.StandardButton.Ok,
        )
        return False
    return True
...
def get_save_file_name(parent, title, default_path, filter_str):
    ...
    output_file, selected_filter = QFileDialog.getSaveFileName(parent, title, default_path, filter_str)
    ...
    if not file_path.suffix and extension:
        ...
        if file_path.suffix not in valid_extensions:
            QMessageBox.critical(
                parent,
                "AcATaMa",
                "Error: The file extension is not valid:\n\n {}\n\nValid extensions are: {}".format(
                    output_file, " ".join(valid_extensions)
                ),
                QMessageBox.StandardButton.Ok,
            )
            return None
